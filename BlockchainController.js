...
class BlockchainController {
    ...
    /**
     * Added so that we can test the validation endpoint. Call to check if the chain is valid.
     */
    validateChain() {
        this.app.get("/validate", async (req, res) => {
            ...
            if (errors.length === 0) {
                ...
                const outputMessage = errors.reduce((allErrors, current) => {
                    return `${allErrors}\n${current}`;
                }, 'Errors:\n');
                return res.status(200).send(outputMessage);
            }
        });
    }
}
